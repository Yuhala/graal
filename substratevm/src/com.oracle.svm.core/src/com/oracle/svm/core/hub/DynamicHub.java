...
public final class DynamicHub implements JavaKind.FormatWithToString, AnnotatedElement, java.lang.reflect.Type, GenericDeclaration, Serializable,
    ...
    @Substitute @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset) private SoftReference<Target_java_lang_Class_ReflectionData<?>> reflectionData;

    @Substitute @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset) private int classRedefinedCount;

    private Object[] enclosingMethodInfo;
    ...
    @TargetElement(onlyWith = JDK17OrLater.class) @UnknownObjectField(types = {byte[].class}) byte[] recordComponentsEncoding;
...
// Checkstyle: disallow direct annotation access
    ...
    private Object[] getEnclosingMethod0() {
        if (enclosingMethodInfo != null) {
            PredefinedClassesSupport.throwIfUnresolvable(toClass((DynamicHub) enclosingMethodInfo[0]), getClassLoader0());
        }
    ...
    private Field[] getDeclaredFields0(boolean publicOnly) {
        return ImageSingletons.lookup(MethodMetadataDecoder.class).parseFields(this, fieldsEncoding, publicOnly);
    }

    @Substitute
    private Method[] getDeclaredMethods0(boolean publicOnly) {
        return ImageSingletons.lookup(MethodMetadataDecoder.class).parseMethods(this, methodsEncoding, publicOnly);
    }

    @Substitute
    private Constructor<?>[] getDeclaredConstructors0(boolean publicOnly) {
        return ImageSingletons.lookup(MethodMetadataDecoder.class).parseConstructors(this, constructorsEncoding, publicOnly);
    }

    @Substitute
    private Class<?>[] getDeclaredClasses0() {
        return ImageSingletons.lookup(MethodMetadataDecoder.class).parseClasses(this, classesEncoding);
    }
    ...
    @Substitute @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset) private transient volatile Target_java_lang_Class_AnnotationData annotationData;

    @Substitute @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset) private transient volatile AnnotationType annotationType;

    @Substitute
    @TargetElement(onlyWith = JDK17OrLater.class)
    private Class<?>[] getPermittedSubclasses0() {
        return ImageSingletons.lookup(MethodMetadataDecoder.class).parseClasses(this, permittedSubclassesEncoding);
    }

    @Substitute @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset) private Constructor<?> cachedConstructor;
    ...
    @KeepOriginal
    private native Field getField0(String name);
}
